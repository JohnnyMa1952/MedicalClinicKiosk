...
unsigned char handleTouch(ButtonTable* buttonTable){
	...
	while(1){
		...
		// needed because of current touch screen controller configurations (remove when fixed)
		...
		for(i=0; i < buttonTable->length; i++){
			...
			if(b->inside(touchPoint)){
				if(prev == NULL)
					prev = b;
				if(is_press && prev_press != b){
					prev_press = b;
					b->press();
				}
				else if(!is_press){
					release_ret = b->release();
					if(release_ret == REDRAW_ALL_ON_RELEASE)
						drawButtonTable(buttonTable);
					else if (release_ret == REDRAW_THIS_ON_RELEASE)
						b->draw();
					prev_press = NULL;
				}
			}
			...
			else if(b == prev_press)
				prev_press = NULL;
		}

		TouchScreenFlush();

		if(release_ret != REDRAW_THIS_ON_RELEASE && release_ret != RELEASE_SAME_SCREEN && release_ret != REDRAW_ALL_ON_RELEASE)
			return release_ret;
	}
