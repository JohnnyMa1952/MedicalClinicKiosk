...
int getTime(void) {
	...
	while(i < 1000){
		...
		if(i > 4 && strArray[i] == '$') {
			...
			if(strArray[0] == 'G' && strArray[1] == 'P' && strArray[2] == 'G' && strArray[3] == 'G' && strArray[4] == 'A') {
				// get hour
				unsigned char hour = ((strArray[6] << 4) & 0xF0) | (strArray[7] & 0x0F);
				unsigned char minute = ((strArray[8] << 4) & 0xF0) | (strArray[9] & 0x0F);
				unsigned char second = ((strArray[10] << 4) & 0xF0) | (strArray[11] & 0x0F);
				...
				sprintf(timeString, "%d%d:%d%d:%d%d", ((int) ((0xf0 & hour) >> 4)), ((int) 0x0f & hour),
