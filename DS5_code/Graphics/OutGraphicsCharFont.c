...
Point OutGraphicsStringFont3(Point p, int colour, int backgroundcolour, char* s, int Erase, int scale){
	int i = 0;
	while(s[i] != '\0'){
		OutGraphicsCharFont3(p.x + i*(FONT3_XPIXELS+1)*scale, p.y, colour, backgroundcolour, (int) s[i], Erase, scale);
		i++;
	}
...
Point OutGraphicsStringFont4(Point p, int colour, int backgroundcolour, char* s, int Erase, int scale){
	int i = 0;
	while(s[i] != '\0'){
		OutGraphicsCharFont4(p.x + i*(FONT4_XPIXELS + 1)*scale, p.y, colour, backgroundcolour, (int) s[i], Erase, scale);
		i++;
	}
...
Point OutGraphicsStringFont5(Point p, int colour, int backgroundcolour, char* s, int Erase, int scale){
	int i = 0;
	while(s[i] != '\0'){
		OutGraphicsCharFont5(p.x + i*(FONT5_XPIXELS + 1)*scale, p.y, colour, backgroundcolour, (int) s[i], Erase, scale);
		i++;
	}
...
int GraphicsStringLength(char* s, int font, int scale){
	register int length;
	register int i;
	...
	{
		case 1:
			length = i*scale*(FONT1_XPIXELS + 1);
			break;
		case 2:
			length = i*scale*(FONT2_XPIXELS + 1);
			break;
		case 3:
			length = i*scale*(FONT3_XPIXELS + 1);
			break;
		case 4:
			length = i*scale*(FONT4_XPIXELS + 1);
			break;
		case 5:
			length = i*scale*(FONT5_XPIXELS + 1);
			break;
		default:
			return -1;
	}
